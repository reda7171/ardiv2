...
if( !function_exists('houzez_register') ) {
    function houzez_register() {
        ...
        if ( is_wp_error($user_id) ) {
            ...
            $is_approval_enabled = houzez_login_option('enable_user_approval', 0);

            if ($is_approval_enabled) {
                // User approval is enabled, show pending approval message
                if( $enable_password == 'yes' ) {
                    echo json_encode( array( 'success' => true, 'msg' => esc_html__('Your account was created successfully! Please wait for admin approval before you can login.', 'houzez-login-register') ) );
                } else {
                    echo json_encode( array( 'success' => true, 'msg' => esc_html__('An email with the generated password was sent! Please wait for admin approval before you can login.', 'houzez-login-register') ) );
                }
            } else {
                // User approval is disabled, show standard message
                if( $enable_password == 'yes' ) {
            ...
            houzez_wp_new_user_notification( $user_id, $user_password, $phone_number, $user_role );
        ...
        if ( is_wp_error($user_id) ) {
            ...
            if ($is_approval_enabled) {
                ...
                echo json_encode( array( 
                    'success' => true,
                    'redirect_to' => esc_url($_POST['redirect_to']),
                    'msg' => esc_html__('Your account was created successfully! Please wait for admin approval before you can login.', 'houzez-login-register') 
                ) );
                
                // Remove the auto-login action if approval is required
                remove_action('houzez_after_social_create_account', 'houzez_social_auto_login');
            } else {
                echo json_encode( array( 
                    'success' => true,
                    'redirect_to' => esc_url($_POST['redirect_to']),
...
if( !function_exists('houzez_link_account') ) {
    function houzez_link_account() {
        ...
        $remember = isset( $_POST['remember'] ) ? $_POST['remember'] : ""; // Initialize remember variable
        $allowed_html_array = array('strong' => array()); // Initialize allowed_html_array variable
...
if( !function_exists('houzez_agency_agent') ) {
    function houzez_agency_agent() {
        ...
        if ( is_wp_error($user_id) ) {
            ...
            if($aa_notification == 'on') {
                houzez_wp_new_user_notification( $user_id, $user_password, null );
            }
        }
        wp_die();
...
if( !function_exists('houzez_wp_new_user_notification') ) {
    ...
    function houzez_wp_new_user_notification( $user_id, $randonpassword = '', $phone_number = '', $user_role = '' ) {
        ...
        $auto_approved_roles = houzez_login_option('auto_approved_roles', array());
        ...
        if( houzez_option('nru_admin_email', 1) != 0 ) {
            ...
            if( houzez_login_option('enable_user_approval', 0) && ! in_array($user_role, $auto_approved_roles) ) {
                // Create a link to the user in the WordPress admin
                $admin_user_link = admin_url('users.php?s=' . urlencode($user_email));
                $args['admin_user_link'] = '<a href="' . esc_url($admin_user_link) . '">' . esc_url($admin_user_link) . '</a>';
                houzez_register_email_type( get_option('admin_email'), 'admin_user_register_approval', $args );
            } else {
...
add_action( 'wp_ajax_nopriv_houzez_forgot_password', 'houzez_forgot_password' );
add_action( 'wp_ajax_houzez_forgot_password', 'houzez_forgot_password' );
if( !function_exists('houzez_forgot_password') ) {
    function houzez_forgot_password() {
...
if( !function_exists('houzez_register_as_agency') ) {
    ...
    function houzez_register_as_agency( $username, $email, $user_id, $phone_number = null ) {
        ...
        if( houzez_login_option('enable_user_approval', 0) ) {
